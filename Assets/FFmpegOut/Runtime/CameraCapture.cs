...
namespace FFmpegOut
{
    ...
    public sealed class CameraCapture : MonoBehaviour
    {
        ...
        [SerializeField] string _outputPath = "output.mp4"; // Default output path
        ...
        [SerializeField] bool _manualCameraRender;

        public bool manualCameraRender
        {
            get { return _manualCameraRender; }
            set { _manualCameraRender = value; }
        }
        ...
        void Update()
        {
            ...
            if (_session == null)
            {
                ...
                _session = FFmpegSession.Create(
                    //gameObject.name,
                    _outputPath, // Pass the output path here
        ...
        void RenderCameraIfNeeded(Camera camera)
        {
            if (_manualCameraRender && camera != null)
            {
                camera.Render();
            }
        }
        ...
        {
            // Close and dispose of the FFmpeg session
            if (_session != null)
            {
                _session.Close();
                _session.Dispose();
            ...
            Debug.Log("Recording has been stopped.(CameraCapture.cs)");
        }
        ...
        public void StopRecording()
        {
            if (_session != null)
            {
                // Ensure all frame data has been pushed to the FFmpeg pipe
                _session.CompletePushFrames();

                // Process all remaining frames in the queue
                _session.ProcessAllFrames();

                // Close the FFmpeg pipe and finalize the video file
                _session.Close();
            ...
            Debug.Log("Recording has been stopped and file has been written.");
        }
